...
$('.find-ID-Form').submit(async function (event) {
    ...
    try {
        ...
        if (snapshot.exists()) {
            ...
            const scriptURL = "https://script.google.com/macros/s/AKfycbxbGOwnsSJrOgCCmmcqxcx2rrXNmpF_4933SGDeAsjD4OvYD6W8NgYi-iWX7RkV7u6gfw/exec";

            const response = await fetch(scriptURL, {
                method: "POST",
                headers: { "Content-Type": "application/x-www-form-urlencoded" },
                body: new URLSearchParams({
                    subject: 'AscentLime 아이디 찾기 인증 코드',
